-- Modern FadelSM v1 — Working FOV Circle, CFrame Speed Hack, Dead Skeleton Fix,
...
if not Executor.Drawing then
    warn("[FADELSM] Drawing API unsupported. ESP disabled.")
end
...
-------------------------------------------------
local oldGui = game.CoreGui:FindFirstChild("FadelSM")
if oldGui then pcall(function() oldGui:Destroy() end) end
if Executor.GetHUI then
    for _, v in ipairs(gethui():GetChildren()) do
        if v.Name == "FadelSM" then pcall(function() v:Destroy() end) end
    end
end
...
subText.Text = "v1 | Circle + Speed Fix"
